...
import { isActiveNode, isUndirectConstraintNode } from 'features/project/work/components/GridPanels/Graph/utils';
...
CRp.drawNode = function (
  ...
  if (
    (!node._private.classes.has('eh-handle') && (isUndirectConstraintNode(node) || !isActiveNode(node))) ||
    _p.data.type === GRAPH_NODE_TYPE.ANGLE
  ) {
    setupShapeColor(0);
  } else {
    setupShapeColor();
  ...
  r.drawAngle(context, node);

  const { _size, lineWidth, type } = _p.data;

  if (type === GRAPH_NODE_TYPE.GROUP) {
    let path1 = new Path2D();
    path1.arc(0, 0, _size / 2 - 2 * lineWidth, 0, 2 * Math.PI);
...
CRp.drawAngle = (context, node) => {
  const { type, angle, _size } = node._private.data;
  if (type === GRAPH_NODE_TYPE.ANGLE) {
    let path = new Path2D();
    path.arc(0, 0, _size / 2, angle.start, angle.end, angle.antiClockWise);
    path.strokeStyle = '#444444';
    context.stroke(path);
  }
};
