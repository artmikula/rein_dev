...
CRp.createGradientStyleFor = function (context, shapeStyleName, ele, fill, opacity) {
  ...
  for (let i = 0; i < length; i++) {
    gradientStyle.addColorStop(
      hasPositions ? positions[i] : i / (length - 1),
      'rgba(' + colors[i][0] + ',' + colors[i][1] + ',' + colors[i][2] + ',' + opacity + ')'
    );
  }
...
CRp.render = function (options) {
  ...
  var textureDraw =
    r.textureOnViewport &&
    !forcedContext &&
    (r.pinching || r.hoverData.dragging || r.swipePanning || r.data.wheelZooming);
  ...
  var viewportIsDiff =
    prevVp === undefined || vp.zoom !== prevVp.zoom || vp.pan.x !== prevVp.pan.x || vp.pan.y !== prevVp.pan.y;
  ...
  function setContextTransform(context, clear) {
    ...
    if (
      ...
      (context === data.bufferContexts[r.MOTIONBLUR_BUFFER_NODE] ||
        context === data.bufferContexts[r.MOTIONBLUR_BUFFER_DRAG])
    ) {
      ePan = {
        x: pan.x * mbPxRatio,
        y: pan.y * mbPxRatio,
  ...
  if (needDraw[r.NODE] || drawAllLayers || drawOnlyNodeLayer || needMbClear[r.NODE]) {
    ...
    var context =
      forcedContext || (useBuffer ? r.data.bufferContexts[r.MOTIONBLUR_BUFFER_NODE] : data.contexts[r.NODE]);
  ...
  if (!drawOnlyNodeLayer && (needDraw[r.DRAG] || drawAllLayers || needMbClear[r.DRAG])) {
    ...
    var context =
      forcedContext || (useBuffer ? r.data.bufferContexts[r.MOTIONBLUR_BUFFER_DRAG] : data.contexts[r.DRAG]);
