  ...
  if (edgeType === 'node') {
    ...
    let source2inter = cy.add(
      getEleJson(
        ...
        options.edgeParams(source, target, 0, this.isNotRelation),
        classes
      )
    );
    ...
    let inter2target = cy.add(
      getEleJson(
        ...
        options.edgeParams(source, target, 1, this.isNotRelation),
        classes
      )
    );
...
function setHandleFor(node) {
  ...
  let handlePosition =
    typeof options.handlePosition === typeof '' ? () => options.handlePosition : options.handlePosition;
...
function updateEdge() {
  ...
  if (!ghostNode || ghostNode.length === 0 || ghostNode.removed()) {
    ...
    cy.batch(() => {
      ...
      let ghostEdgeParams = options.ghostEdgeParams(sourceNode, ghostNode, this.isNotRelation);
